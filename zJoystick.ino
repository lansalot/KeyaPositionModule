...
int CANInfo[3] = { 0x18FF2005 , 5, 0x04}; // Valtra engage gen5 button 
...
CAN_message_t JSData;

void init_canbus1_filters()
{
    canbus1.enableFIFO();
    ...
    {
        Serial.print("Setting extended filter 0x");
        Serial.println(CANInfo[filterID], HEX);
        canbus1.setFIFOFilter(0, CANInfo[filterID], EXT);
    }
    else
    {
        Serial.print("Setting standard filter 0x");
        Serial.println(CANInfo[filterID], HEX);
        canbus1.setFIFOFilter(0, CANInfo[filterID], STD);
    }
    delay(500);
}

void JoystickBus_Receive() {
	if (canbus1.read(JSData)) {
		...
		if (debugCANBUS) {
			...
			Serial.print(JSData.id, HEX); Serial.print("  ");
			Serial.print("Byte of interest at offset "); Serial.print(CANInfo[byteOffset]); Serial.print("  Value: ");
			Serial.println(JSData.buf[CANInfo[byteOffset]], HEX);
		}
		if ((JSData.buf[CANInfo[byteOffset]] & CANInfo[ANDValue]) && millis() - lastCANCommand > 1000) {

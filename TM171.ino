    ...
    if (gotPacket)
    {
      ...
      if (GoodCRC(ImuData, ImuData[2] + 5))
      {
        ...
        case 35: // RPY Output du TM171
        {
          ...
          // 1. Calculate base gain via user1 (e.g. calibrated for 5 km/h)
          // Assumes user1 gives the desired responsiveness at low speed.
          // float baseGain = (float)aogConfig.user1 / 255.0f;
          float baseGain = 0.1f + ((float)aogConfig.user1 / 255.0f) * 0.49f;
          if (baseGain < 0.01f)
            baseGain = 0.01f;

          // 2. Calculate Dynamic Gain (Inverted)
          // Reduce gain as speed increases.
          // Formula: Gain = BaseGain * (5.0 / gpsSpeed)
          float dynamicGain;
          if (gpsSpeed < 1.0f)
          {
            dynamicGain = baseGain; // Max responsiveness at standstill
          }
          else
          {
          ...
          // 3. Apply limits (Max smoothing 0.5 at 18km/h as requested)
          // For a "stronger" filter at 18km/h, gain must be SMALLER.
          // Clamp gain so it does not drop below a too-slow value.
          if (dynamicGain > 1.0f)
            dynamicGain = 1.0f;
          if (gpsSpeed >= 18.0f && dynamicGain > 0.1f)
            dynamicGain = 0.1f; // Strong filter at high speed
          ...
          YawV.fValue = rawYaw; // No filter applied on heading
          RollV.fValue = fRoll;
